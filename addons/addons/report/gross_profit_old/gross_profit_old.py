...
def execute(filters=None):
	...
	data = []
	group_wise_columns=["parent","item_code","item_name", "customer", "customer_group","territory","sales_person", "posting_date","item_code", "item_name","item_group", "brand", "description", \
				...
				for i, sle in enumerate(my_sle):
					...
					if sle.voucher_type == parenttype and parent == sle.voucher_no and \
						sle.voucher_detail_no == row.item_row:
							...
							return sle.valuation_rate * sle.qty *-1
...
#				return -1
	...
	def get_last_purchase_rate(self, item_code, row):
		...
		select (a.base_net_rate / a.conversion_factor)
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice Item`.base_net_rate as "base_rate", `tabSales Invoice Item`.base_net_amount as "base_amount",
			...
			from
				...
				{sales_team_table}
			where
				sales.`parenttype` = 'Sales Invoice' and `tabSales Invoice`.docstatus=1 and `tabSales Invoice`.is_opening!='Yes' {conditions} {match_cond}
			order by
	...
	def get_invoice_row(self, row):
		return frappe._dict({
			...
			'base_amount': frappe.db.get_value('Sales Invoice', row.parent, 'base_total')
		})
	...
	def load_stock_ledger_entries(self):
		res = frappe.db.sql("""select item_code, voucher_type, voucher_no,
				voucher_detail_no, stock_value, warehouse, actual_qty as qty , valuation_rate
